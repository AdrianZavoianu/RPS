...
class ReportSectionLoader:
    ...
    def _fetch_section_data(self, section: ReportSection) -> object:
        ...
        if section.category == "Element":
            ...
            if section.result_type == "WallShears":
                return self.reporting_service.get_wall_shear_data(
                    self.project_id,
                    section.result_set_id,
                    self.analysis_context,
                )
            if section.result_type == "QuadRotations":
                return self.reporting_service.get_quad_rotation_data(
                    self.project_id,
                    section.result_set_id,
                    self.analysis_context,
                )
            if section.result_type == "ColumnShears":
                return self.reporting_service.get_column_shear_data(
                    self.project_id,
                    section.result_set_id,
                    self.analysis_context,
                )
            if section.result_type == "ColumnAxials":
                return self.reporting_service.get_column_axial_data(
                    self.project_id,
                    section.result_set_id,
                    self.analysis_context,
                )
            if section.result_type == "BraceAxials":
                return self.reporting_service.get_brace_axial_data(
                    self.project_id,
                    section.result_set_id,
                    self.analysis_context,
                )
        ...
        if section.category == "Joint":
            ...
            if section.result_type == "VerticalDisplacements_Min":
                return self._fetch_vertical_displacement_data(section.result_set_id)
            return None
    ...
    def _fetch_vertical_displacement_data(self, result_set_id: int) -> Optional[dict]:
        """Fetch vertical displacement data for reporting."""
        is_pushover = self.analysis_context == "Pushover"
        dataset = self.result_service.get_joint_dataset(
        ...
        # Add Avg, Max, Min if not already present
        if "Avg" not in df.columns and self.analysis_context != "Pushover":
            df["Avg"] = numeric_df.abs().mean(axis=1)
        if "Max" not in df.columns:
            df["Max"] = numeric_df.abs().max(axis=1)
        ...
        # Prepare scatter plot data: (load_case_index, value)
        plot_data = []
        for lc_idx, lc in enumerate(load_case_cols):
            if lc in numeric_df.columns:
                values = numeric_df[lc].dropna().abs().values
