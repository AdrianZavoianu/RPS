    ...
    def refresh(self):
        ...
        try:
            ...
            for project in projects:
                ...
                project_item = QTreeWidgetItem(self.tree, [f"â–¸ {project.name}"])
                ...
                if load_cases:
                    cases_node = QTreeWidgetItem(project_item, [f"â—† Load Cases ({len(load_cases)})"])
                    cases_node.setExpanded(False)  # Collapsed by default

                    for case in load_cases:
                        case_display = f"  {case.name}"
                        if case.case_type:
                            case_display += f" â€¢ {case.case_type}"
                        case_item = QTreeWidgetItem(cases_node, [case_display])

                # Results node
                results_node = QTreeWidgetItem(project_item, ["â—‡ Results"])
