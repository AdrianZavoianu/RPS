...
class ResultViewController:
    ...
    def __init__(self, project_id: int, cache_repo, controller) -> None:
    ...
    def get_standard_view(
        self, result_service, result_type: str, direction: str, result_set_id: int
    ) -> Tuple[Any, Optional[dict]]:
        """Fetch standard dataset and optional mapping."""
        dataset = result_service.get_standard_dataset(result_type, direction, result_set_id)
        mapping = self.get_pushover_mapping(result_set_id) if self.should_apply_pushover_mapping() else None
        return dataset, mapping

    def get_element_view(
        self, result_service, element_id: int, result_type: str, direction: str, result_set_id: int
    ) -> Tuple[Any, Optional[dict]]:
        """Fetch element dataset and optional mapping."""
        dataset = result_service.get_element_dataset(element_id, result_type, direction, result_set_id)
        mapping = self.get_pushover_mapping(result_set_id) if self.should_apply_pushover_mapping() else None
        return dataset, mapping

    def get_joint_view(
        self, result_service, result_type: str, result_set_id: int
    ) -> Tuple[Any, Optional[dict]]:
        """Fetch joint dataset and optional mapping."""
        dataset = result_service.get_joint_dataset(result_type, result_set_id)
        mapping = self.get_pushover_mapping(result_set_id) if self.should_apply_pushover_mapping() else None
        return dataset, mapping
