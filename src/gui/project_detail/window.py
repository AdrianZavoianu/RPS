...
class ProjectDetailWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Central widget
        central_view = ProjectDetailWindowView()
        self.setCentralWidget(central_view)
        layout = central_view.layout

        # Header bar (modular component)
        self.header = ProjectHeader(
            self.project_name,
            on_switch_context=self._switch_context,
            on_load_nltha=self.load_data_from_folder,
            on_load_time_series=self.load_time_series,
            on_create_comparison=self.create_comparison_set,
            on_export_nltha=self.export_results,
            on_load_pushover_curves=self.load_pushover_curves,
            on_load_pushover_results=self.load_pushover_results,
            on_export_pushover=self.export_results,
            on_export_project=self.export_project_excel,
            on_settings=self._show_settings_popup,
            on_open_reporting=self.open_reporting,
            on_open_pushover_reporting=self.open_pushover_reporting,
        )
        self.header.set_context("NLTHA")
        layout.addWidget(self.header)

        # Spacing below header (stored for border mode adjustment)
        self._header_spacer = central_view.header_spacer
        layout.addWidget(self._header_spacer)

        # Main content splitter (browser | table + plots)
        self._main_splitter = central_view.main_splitter
        ...
        self._content_area = build_content_area()
        self._content_widget = self._content_area.widget

        # Expose views for existing handlers
        self.content_title = self._content_area.content_title
        self.standard_view = self._content_area.standard_view
        self.comparison_view = self._content_area.comparison_view
        self.maxmin_widget = self._content_area.maxmin_widget
        self.all_rotations_widget = self._content_area.all_rotations_widget
        self.comparison_all_rotations_widget = self._content_area.comparison_all_rotations_widget
        self.comparison_joint_scatter_widget = self._content_area.comparison_joint_scatter_widget
        self.soil_pressure_plot_widget = self._content_area.soil_pressure_plot_widget
        self.beam_rotations_table = self._content_area.beam_rotations_table
        self.pushover_curve_view = self._content_area.pushover_curve_view
    ...
    def content_area(self) -> ContentArea:
        """Expose content area for view loaders."""
    ...
    def _show_settings_popup(self):
        ...
        popup.show_below(self.header.settings_button)
    ...
    def _apply_layout_borders(self):
        ...
        if settings.layout_borders_enabled:
            # Convert spacer to border line (1px height with border color)
            self._header_spacer.setFixedHeight(1)
            self._header_spacer.setStyleSheet(f"background-color: {border_color};")

            # Header: padding adjustment (no CSS border - using spacer as border)
            ...
            # Restore spacer below header (transparent)
            self._header_spacer.setFixedHeight(8)
            self._header_spacer.setStyleSheet("background-color: transparent;")
