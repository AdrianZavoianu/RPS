...
IMPORTER_REGISTRY: Dict[str, tuple[str, str]] = {
    ...
    "column_rotation": (
        "processing.pushover.pushover_column_importer_v2",
        "PushoverColumnImporter",
    ),
    "column_shear": (
        "processing.pushover.pushover_column_shear_importer_v2",
        "PushoverColumnShearImporter",
    ),
    "beam_rotation": ("processing.pushover.pushover_beam_importer_v2", "PushoverBeamImporter"),
    "brace_axial": ("processing.pushover.pushover_brace_importer", "PushoverBraceImporter"),
    # Joint importers
    "joint_displacement": ("processing.pushover.pushover_joint_importer", "PushoverJointImporter"),
    "soil_pressure": (
        "processing.pushover.pushover_soil_pressure_importer",
        "PushoverSoilPressureImporter",
    ),
    "vertical_displacement": (
        "processing.pushover.pushover_vert_displacement_importer",
...
def get_importer_class(importer_type: str) -> Type:
    ...
    if importer_type not in IMPORTER_REGISTRY:
        ...
        raise KeyError(f"Unknown importer type: {importer_type}. " f"Available: {available}")
    ...
    try:
        ...
        raise ImportError(f"Failed to load importer {class_name} from {module_path}: {e}") from e
...
def get_element_importers() -> List[str]:
    ...
    return ["wall", "column_rotation", "column_shear", "beam_rotation", "brace_axial"]
...
def import_multiple(
    ...
    for i, importer_type in enumerate(importer_types):
        ...
        if progress_callback:
            progress_callback(f"Importing {importer_type}...", int((i / total) * 100), 100)
