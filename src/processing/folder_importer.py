...
class EnhancedFolderImporter(BaseFolderImporter):
    ...
    def import_all(self) -> Dict[str, Any]:
        ...
        # Determine load cases to import
        if self.selected_load_cases is None:
            all_load_cases, load_case_sources = self._collect_load_case_sources(file_load_cases)
            if self.selection_provider:
                selected_load_cases = self.selection_provider(
                    all_load_cases,
                    load_case_sources,
                    self.result_set_name,
                )
                if selected_load_cases is None:
                    return {
                        "project": None,
                        "files_processed": 0,
                        "files_total": len(self.excel_files),
                        "load_cases": 0,
                        "errors": ["Load case selection cancelled"],
                        "phase_timings": [],
                    }
            else:
                selected_load_cases = all_load_cases
        else:
            selected_load_cases = self.selected_load_cases

        # Resolve conflicts if requested and no resolution provided
    ...
    def _collect_load_case_sources(
        ...
        """Collect all load cases and their originating (file, sheet) pairs."""
        all_load_cases: Set[str] = set()
        ...
        for file_name, sheets in file_load_cases.items():
            for sheet_name, load_cases in sheets.items():
                for lc in load_cases:
                    ...
                    load_case_sources.setdefault(lc, []).append((file_name, sheet_name))

        return all_load_cases, load_case_sources
    ...
    # Interactive conflict resolution is handled by injected conflict_resolver.
